...
class DiagnosisViewModel: ViewModel() {
    ...
    fun getQuestionList() {
        ...
        testList.add(QuestionResponse(3, "일을 하거나 놀이를 할 때 지속적으로 주의를 집중하는데에 어려움이 있다.", "수업시간에 딴 생각에 빠져 있는다."))
        ...
        testList.add(QuestionResponse(9,"과제와 일을 체계적으로 하지 못한다.","순차적인 과제를 처리하는 데에 어려움을 느낀다. 물건이나 소지품을 정리하는 데에 어려움을 느낀다. 시간 관리를 잘 하지 못한다. 마감시간을 맞추지 못한다."))
        testList.add(QuestionResponse(10,"끊임없이 무엇인가를 하거나 마치 모터가 돌아가듯 움직인다.","음식점 같은 곳에서 장시간 동안 가만히 있을 수 없거나 불편해한다."))
        testList.add(QuestionResponse(11,"지속적인 노력이 요구되는 과제(학교 공부나 숙제)를 하지 않으려 한다.",""))
        ...
        testList.add(QuestionResponse(15,"쉽게 산만해진다.",""))
        testList.add(QuestionResponse(16,"차례를 기다리는 데에 어려움이 있다.","줄을 서 있는 동안 기다림에 어려움을 겪는다."))
